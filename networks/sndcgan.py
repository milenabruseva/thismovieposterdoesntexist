    ...
    def __init__(self, num_img_channels, base_num_out_channels, padding_mode):
        ...
        self.main = nn.Sequential(
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(num_img_channels, base_num_out_channels, kernel_size=3, stride=1, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels, base_num_out_channels, kernel_size=4, stride=2, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels, base_num_out_channels * 2, kernel_size=3, stride=1, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 2, base_num_out_channels * 2, kernel_size=4, stride=2, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 2, base_num_out_channels * 4, kernel_size=3, stride=1, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 4, base_num_out_channels * 4, kernel_size=4, stride=2, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 4, base_num_out_channels * 8, kernel_size=3, stride=1, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 8, base_num_out_channels * 8, kernel_size=4, stride=2, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 8, base_num_out_channels * 16, kernel_size=3, stride=1, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.utils.parametrizations.spectral_norm(
                nn.Conv2d(base_num_out_channels * 16, base_num_out_channels * 16, kernel_size=4, stride=2, padding=1,
                          padding_mode=padding_mode, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Flatten(),
...
#         self.main = nn.Sequential(
#             nn.utils.parametrizations.spectral_norm(
#                 nn.Conv2d(num_img_channels, base_num_out_channels, kernel_size=(4, 4), stride=(2, 2),
#                           padding=(1, 1),
#                           padding_mode=padding_mode, bias=True)),
#             nn.LeakyReLU(0.1, inplace=True),
#             nn.utils.parametrizations.spectral_norm(
#                 nn.Conv2d(base_num_out_channels, base_num_out_channels * 2, kernel_size=(4, 4), stride=(2, 2),
#                           padding=(1, 1), padding_mode=padding_mode, bias=True)),
#             nn.LeakyReLU(0.1, inplace=True),
#             nn.utils.parametrizations.spectral_norm(
...
#             nn.LeakyReLU(0.1, inplace=True),
#             nn.utils.parametrizations.spectral_norm(
#                 nn.Conv2d(base_num_out_channels * 8, 1, kernel_size=(4, 4), stride=(1, 1), bias=True)),
#             nn.LeakyReLU(0.1, inplace=True),
#             nn.Flatten(),
#             nn.Linear(1 * 3 * 1, 1),
#             nn.Sigmoid()
